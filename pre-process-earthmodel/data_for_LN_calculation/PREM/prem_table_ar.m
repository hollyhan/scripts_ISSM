...
% computes the seismic properties for a single depth value based on PREM.
% Input depth in km, (seismic) period in seconds
% crust: indicator whether the crust is continental (1) or oceanic (0)
...
vsv1 = vsv*(1-log(period)/(pi*Qmu)); % Vertically polarized S-wave velocity.
vsh1 = vsh*(1-log(period)/(pi*Qmu)); % Horizontally polarized S-wave velocity.
vpv1 = vpv*(1-log(period)/pi*((1-E)/Qmu+E/Qk)); % Vertically polarized P-wave velocity.
vph1 = vph*(1-log(period)/pi*((1-E)/Qmu+E/Qk)); % Horizontally polarized P-wave velocity.
